...
void ColumnFill(uint8_t roundkeys[][STATE_ROW_SIZE][STATE_COL_SIZE], int round){
    uint8_t wi_minus_4[STATE_COL_SIZE] = {roundkeys[round][0][0], 
                                    roundkeys[round][1][0], 
                                    roundkeys[round][2][0], 
                                    roundkeys[round][3][0]
                                };

    uint8_t wi[STATE_COL_SIZE] = {roundkeys[round][1][3], 
                                roundkeys[round][2][3], 
                                roundkeys[round][3][3], 
                                roundkeys[round][0][3]
                            };
    ...
    for(int i = 0; i < STATE_COL_SIZE; i++){
        uint8_t high_nibble = (wi[i] >> 4) & 0x0F;
        uint8_t low_nibble = wi[i] & 0x0F; 
        wi[i] = sboxtab[(int)high_nibble * 16 + (int)low_nibble];
    }

    for(int i=0; i < STATE_ROW_SIZE; i++){
        roundkeys[round][i][0] = wi[i] ^ wi_minus_4[i] ^ (i == 0 ? rcon[round] : 0);
    }
}
...
void OtherColumnsFill(uint8_t roundkeys[][STATE_ROW_SIZE][STATE_COL_SIZE], int round){
    for(int i=0; i < STATE_ROW_SIZE; i++){
        for(int j=1; j < STATE_COL_SIZE; j++){
            roundkeys[round][i][j] = roundkeys[round][i][j-1] ^ roundkeys[round][i][j];
        }
    }
}
...
void MCMatrixColumnProduct(uint8_t colonne[STATE_COL_SIZE]){
    const uint8_t mixColumnsMatrix[DATA_SIZE] = {
      0x02, 0x03, 0x01, 0x01,
      0x01, 0x02, 0x03, 0x01,
      0x01, 0x01, 0x02, 0x03,
      0x03, 0x01, 0x01, 0x02
    };
    const uint8_t temp[STATE_COL_SIZE] = {colonne[0], colonne[1], colonne[2], colonne[3]};

    for(int i=0; i < STATE_COL_SIZE; i++){
        colonne[i] = gmul(temp[0], mixColumnsMatrix[0+(i*4)]) 
                ^ gmul(temp[1], mixColumnsMatrix[1+(i*4)])
                ^ gmul(temp[2], mixColumnsMatrix[2+(i*4)])
                ^ gmul(temp[3], mixColumnsMatrix[3+(i*4)]);
    }
}
